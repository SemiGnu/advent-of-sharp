...
let scoreHand2 (cards:string, bet:int) = let joker = nonJokers |> Seq.sortBy (fun c -> scoreHand (cards.Replace('1',c),bet)) |> Seq.last
                                         scoreHand (cards.Replace('1',joker),bet)
let winnings2 lines = lines |> Seq.map parseHand2 |> Seq.sortBy (scoreHand2 >> fst) |> Seq.mapi (fun i (_,bet) -> bet*(i+1)) |> Seq.sum 
